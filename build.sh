...
usage: ./build.sh [--version VERSION|--build BUILD|--url IPSW_URL]
                  [--product PRODUCT --model BOARD --cpid CPID]
                  [--list] [--im4m PATH]
...
If neither --version nor --build is given, an interactive firmware picker runs.
  ...
  --product      offline product identifier, for example iPhone11,8
  --model        offline board/DeviceClass, for example n841ap
  --cpid         offline chip ID: 0x8020, 0x8027, or 0x8030
  --name         optional display name used with offline device parameters
...
The device is not required when --product, --model, and --cpid are all given.
EOF
}
...
if [[ -n "$OFFLINE_PRODUCT$OFFLINE_MODEL$OFFLINE_CPID" ]]; then
    [[ -n "$OFFLINE_PRODUCT" && -n "$OFFLINE_MODEL" && -n "$OFFLINE_CPID" ]] || {
        echo "--product, --model, and --cpid must be provided together" >&2
        exit 64
    }
    OFFLINE=1
fi
...
REQUIRED_TOOLS=("$PZB" "$IMG4" "$GTAR" "$TC" "$JQ" curl ipsw python3 hdiutil diskutil)
((OFFLINE)) || REQUIRED_TOOLS=("$IRECOVERY" "${REQUIRED_TOOLS[@]}")
...
if ((OFFLINE)); then
    ...
    PWND="not-required"
    ECID=""
    NAME="${OFFLINE_NAME:-offline target}"
else
